...
export default function SignaturePage() {
  ...
  // ===== Helpers (ảnh/chữ ký) =====
  async function trimTransparentPng(dataUrl: string, padding = 6): Promise<string> {
    const img = new Image();
    img.src = dataUrl;
    ...
    c2.width = newW;
    c2.height = newH;
    c2.getContext("2d")!.drawImage(c, left, top, newW, newH, 0, 0, newW, newH);
  ...
  async function toBWSignature(dataUrl: string): Promise<string> {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      if (Y >= WHITE_THR) {
        ...
        const t = Math.min(1, Math.max(0, (BLACK_THR - Y) / (BLACK_THR - WHITE_THR + 1)));
  ...
  const handleSaveSignature = async () => {
    ...
    try {
      setSaving(true);
      const pngTransparent = await ensureTransparentPng(selectedSignature);
      localStorage.setItem("savedSignature", pngTransparent);
      setSavedSignature(pngTransparent);
      const ts = new Date();
      const fname =
        `signature_${ts.getFullYear()}-${String(ts.getMonth() + 1).padStart(2, "0")}-${String(ts.getDate()).padStart(2, "0")}.png`;
  ...
  const onWrapperPointerMove = (e: React.PointerEvent) => {
    ...
    const rect = docWrapRef.current.getBoundingClientRect();
    const { mouseX, w: startW, h: startH, x: startX, y: startY } = resizeStart.current;
    ...
    let newX = startX;
    let newY = startY;
    if (resizing === "tl" || resizing === "bl") newX = startX + (startW - newW);
    if (resizing === "tl" || resizing === "tr") newY = startY + (startH - newH);
          ...
          {file && (
            <>
              {selectedSignature && (
                ...
                >
                  <FiSave /> {saving ? "Đang lưu..." : "Lưu chữ ký"}
                </button>
              )}
        ...
        <div className="sign-shell">
          ...
          <aside className="left-rail">
            <div className="rail-group">
              <button
                ...
                onClick={() => {
                  setSigTab("draw");
                  setOpenPanel(openPanel === "draw" ? null : "draw");
                }}
              >
                <FiEdit />
              </button>
          ...
          <main className="viewer-area" ref={viewerAreaRef}>
            <div
              ...
              onMouseDown={(e) => {
                if (e.target === e.currentTarget) setSigSelected(false);
              }}
            >
              ...
              {showOnDocument && selectedSignature && (
                <div
                  style={{
                    ...
                    borderRadius: 6,
                    userSelect: "none",
                    outline: sigSelected ? "2px dashed #1e6cff" : "none",
                    boxShadow: sigSelected ? "0 0 0 4px rgba(30,108,255,.12)" : "none",
                  }}
                ...
                >
                  <img
                    ...
                    style={{
                      ...
                      height: "auto",
                      pointerEvents: "auto",
                      filter: dragging ? "drop-shadow(0 2px 8px rgba(0,0,0,.25))" : "none",
                    }}
                  />
                  <span style={handleDotStyle("tl")} onPointerDown={onResizeDown("tl")} />
                  <span style={handleDotStyle("tr")} onPointerDown={onResizeDown("tr")} />
                  <span style={handleDotStyle("bl")} onPointerDown={onResizeDown("bl")} />
                  <span style={handleDotStyle("br")} onPointerDown={onResizeDown("br")} />
                </div>
              )}
            </div>
          </main>
            ...
            {openPanel === "draw" && (
              <>
                ...
                <div className="sig-tabs">
                  <button
                    className={`sig-tab ${sigTab === "draw" ? "active" : ""}`}
                    onClick={() => setSigTab("draw")}
                  >
                    ✍️ Vẽ
                  </button>
                  <button
                    className={`sig-tab ${sigTab === "upload" ? "active" : ""}`}
                      ...
                      <div style={{ marginBottom: 10 }}>
                        <label>
                          ...
                          <input
                            ...
                            step="0.5"
                            value={drawLineWidth}
                            onChange={(e) => setDrawLineWidth(parseFloat(e.target.value))}
                    ...
                    <>
                      ...
                      <div className="sig-work">
                        <div className="upload-pane">
                          {!uploadPreview ? (
                            <div className="upload-drop" onClick={openUploadDialog}>
                              <div className="upload-invite">Bấm để chọn ảnh chữ ký</div>
                              <div className="upload-sub">Chỉ nhận: PNG • JPG • JPEG • WEBP</div>
                            </div>
                          ) : (
                            <div className="upload-preview">
                              <img src={uploadPreview} alt="preview-signature" />
                            </div>
                          )}
                      ...
                      <div className="action-buttons" style={{ justifyContent: "space-between" }}>
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          <button onClick={() => setOpenPanel(null)}>Hủy</button>
                          <button onClick={handleCreateFromUpload} disabled={!uploadPreview}>
                            Tạo
                          </button>
                        </div>
                      </div>
                ...
                <div className="sig-box">
                  <div className="sig-preview">
                    {savedSignature ? (
                      ...
                      <span className="sig-empty">Chưa có chữ ký đã lưu</span>
                    )}
                  </div>
                  <button
...
function handleDotStyle(pos: "tl" | "tr" | "bl" | "br"): React.CSSProperties {
  ...
  if (pos === "tl") return { ...base, left: -6, top: -6, cursor: "nwse-resize" };
  if (pos === "tr") return { ...base, right: -6, top: -6, cursor: "nesw-resize" };
  if (pos === "bl") return { ...base, left: -6, bottom: -6, cursor: "nesw-resize" };
